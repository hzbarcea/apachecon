...
public class DynamicRoutingSlipTest extends CamelTestSupport {
	private static final Logger LOG = LoggerFactory.getLogger(DynamicRoutingSlipTest.class);
    ...
    public void testSimpleInvoke() throws Exception {
		assertInvocationSuccessful("direct:simple",ACON_SERVICE, "direct:one,direct:two,direct:three,mock:apachecon");
	}
    ...
    protected RouteBuilder createRouteBuilder() throws Exception {
        return new RouteBuilder() {
            public void configure() {
                from("direct:simple")
                    .routingSlip(header(ACON_SERVICE));
                ...
                from("direct:resolve-service")
	                .setBody(constant("direct:one,direct:two,direct:three,mock:apachecon"));

                
                from("direct:one")
                    .setBody(simple("${body} Europe"))
                    .to("log:com.apachecon.camel.ONE");

                from("direct:two")
                    .setBody(simple("${body} in Sinsheim, DE"))
                    .to("log:com.apachecon.camel.TWO");

                from("direct:three")
                    .setBody(constant(ACON_BYE))
                    .to("log:com.apachecon.camel.THREE");
            }
        };
    }
	...
	public static Expression hardcoded() {
		return new Expression()  {
			...
			public <T> T evaluate(Exchange exchange, Class<T> type) {
				return (T)"direct:one,direct:two,direct:three,mock:apachecon";
			}
		};
	}
